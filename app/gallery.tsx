...
import {demoPhotos,demoFolders} from './photos';
type FolderItem={id:string;name:string;position:number};
type Photo={id:string;name:string;url:string;size:number;type:string;folderId:string|null};
const API=import.meta.env.VITE_GALLERY_API||'/api/gallery';
const size=(n:number)=>n>1e6?(n/1e6).toFixed(1)+' Mo':Math.round(n/1000)+' Ko';
const MAX_UPLOAD=820*1024;
...
export default function Gallery(){
  ...
  async function login(e:React.FormEvent){e.preventDefault();setBusy(true);try{const r=await fetch(API+'?action=login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code})});const d=await r.json();if(!r.ok)throw Error(d.error);setToken(d.token);setCode('')}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
  async function unlock(e:React.FormEvent){e.preventDefault();setVisitorBusy(true);try{const r=await fetch(API+'?action=access-login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:visitorCode})});const d=await r.json();if(!r.ok)throw Error(d.error);setAccessToken(d.token||'');setVisitorCode('');setAccessMode('open');setLoading(true);await load(d.token||'')}catch(e){toast.error((e as Error).message)}finally{setVisitorBusy(false)}}
  async function saveAccess(enabled:boolean){setBusy(true);try{if(enabled&&!/^\d{4}$/.test(newAccessCode))throw Error('Choisissez un code de 4 chiffres.');await request('access-config',JSON.stringify({enabled,code:newAccessCode}));setAccessEnabled(enabled);setNewAccessCode('');toast.success(enabled?'Code d’accès activé':'Code d’accès désactivé');if(enabled){setAccessToken('');setAccessMode('locked');setPhotos([]);setFolders([])}}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
  async function upload(files:FileList|null){if(!files||busy)return;setBusy(true);let done=0;try{for(const file of Array.from(files)){if(!['image/jpeg','image/png','image/webp'].includes(file.type))throw Error(file.name+' : utilisez un fichier JPG, PNG ou WebP.');if(file.size>50*1024*1024)throw Error(file.name+' dépasse 50 Mo.');setProgress(`Préparation de ${file.name} (${done+1}/${files.length})`);const ready=await prepareUpload(file);setProgress(`Ajout de ${file.name} (${done+1}/${files.length})`);const data=new FormData();data.append('file',ready);data.append('folderId',adminFolder);await request('upload',data);done++}toast.success(`${done} photo${done>1?'s ajoutées':' ajoutée'}`)}catch(e){toast.error((e as Error).message)}finally{await load(accessToken);setBusy(false);setProgress('');if(fileInput.current)fileInput.current.value=''}}
  async function movePhoto(i:number,delta:number){const list=[...managed];[list[i],list[i+delta]]=[list[i+delta],list[i]];setBusy(true);try{await request('reorder',JSON.stringify({ids:list.map(p=>p.id),folderId:adminFolder||null}));await load()}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
  async function moveFolder(i:number,delta:number){const list=[...folders];[list[i],list[i+delta]]=[list[i+delta],list[i]];setBusy(true);try{await request('folder-reorder',JSON.stringify({ids:list.map(f=>f.id)}));setFolders(list)}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
  async function createFolder(e:React.FormEvent){e.preventDefault();if(!newFolder.trim())return;setBusy(true);try{const d=await request('folder-create',JSON.stringify({name:newFolder}));setNewFolder('');await load(accessToken);setAdminFolder(d.id);toast.success('Dossier créé')}catch(e){toast.error((e as Error).message)}finally{setBusy(false)}}
