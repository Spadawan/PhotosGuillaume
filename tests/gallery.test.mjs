...
database.exec(await readFile(new URL('../drizzle/0002_dashing_kate_bishop.sql',import.meta.url),'utf8'));
...
assert.equal((await POST(request('access-config',{enabled:true,code:'1234'}))).status,200);
assert.equal((await GET(new Request('https://album.test/api/gallery'))).status,401);
assert.equal((await POST(request('access-login',{code:'0000'},false))).status,401);
const visitorLogin=await POST(request('access-login',{code:'1234'},false));assert.equal(visitorLogin.status,200);const visitorToken=(await visitorLogin.json()).token;
assert.equal((await GET(new Request('https://album.test/api/gallery',{headers:{Authorization:'Bearer '+visitorToken}}))).status,200);
assert.equal((await POST(request('access-config',{enabled:false}))).status,200);
